...
def wan_ksampler(model_high_noise, model_low_noise, seed, steps, cfgs, sampler_name, scheduler, positive, negative, latent, boundary = 0.875, denoise=1.0, disable_noise=False, start_step=None, last_step=None, force_full_denoise=False, high_steps=99, sigma_shift=None):
...
# Added by Wayland 2025-11-24, to compute sigmas based on reddit post
#  https://www.reddit.com/r/comfyui/comments/1mve9ji/how_to_fix_the_overexposed_burntout_artifacts_in/
def compute_sigmas(shift, steps):
    # For any number of steps:
    timesteps = linspace(1000, 0, steps + 1)
    normalized = timesteps / 1000
    shifted = shift * normalized / (1 + (shift - 1) * normalized)
    sigmas = torch.tensor(shifted.tolist())
    return(sigmas)
...
class WanMoeKSampler:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "high_steps": ("INT", {"default":99, "min": 0, "max": 99, "step": 1, "tooltip": "Manually set the step to switch from high to low model."}),
            }
        }
    ...
    def sample(self, model_high_noise, model_low_noise, boundary, seed, steps, cfg_high_noise, cfg_low_noise, sampler_name, scheduler, sigma_shift, positive, negative, latent_image, denoise=1.0, high_steps=99):
...
class WanMoeKSamplerAdvanced:
    ...
    def sample(self, model_high_noise, model_low_noise, boundary, add_noise, noise_seed, steps, cfg_high_noise, cfg_low_noise, sampler_name, scheduler, sigma_shift, positive, negative, latent_image, start_at_step, end_at_step, return_with_leftover_noise, denoise=1.0):
        ...
        if add_noise == "disable":
            disable_noise = True
        return wan_ksampler(model_high_noise, model_low_noise, noise_seed, steps, (cfg_high_noise, cfg_low_noise), sampler_name, scheduler, positive, negative, latent_image, boundary=boundary, denoise=denoise, disable_noise=disable_noise, start_step=start_at_step, last_step=end_at_step, force_full_denoise=force_full_denoise,  sigma_shift=sigma_shift)
...
class WanMoeKSamplerBasic:
    ...
    def sample(self, model_high_noise, model_low_noise, seed, steps, sampler_name, scheduler, positive, negative, latent_image, high_steps=99):
        boundary = 0.9
        cfg_low_noise = 1.0
        cfg_high_noise = 1.0
        sigma_shift = 5.0
